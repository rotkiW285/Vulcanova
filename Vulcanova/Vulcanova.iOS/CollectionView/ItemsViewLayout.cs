...
namespace Vulcanova.iOS.CollectionView
{
	public abstract class ItemsViewLayout : UICollectionViewFlowLayout
	{
		...
		protected ItemsViewLayout(ItemsLayout itemsLayout,
			ItemSizingStrategy itemSizingStrategy = ItemSizingStrategy.MeasureFirstItem)
		{
		...
		public override bool ShouldInvalidateLayout(UICollectionViewLayoutAttributes preferredAttributes, UICollectionViewLayoutAttributes originalAttributes)
		{
			...
			{
				// On devices with dynamic island the preferred and original height can be insignificantly different
				// causing unnecessary layout invalidate. This magic constant introduces a height diff threshold below
				// which no layout invalidate will be caused.
				const float verticalScrollHeightDiffThreshold = 0.1f;
				var heightRequiresLayoutInvalidate = ScrollDirection == UICollectionViewScrollDirection.Vertical &&
				                                    Math.Abs(preferredAttributes.Bounds.Height -
				                                             originalAttributes.Bounds.Height) > verticalScrollHeightDiffThreshold;

				if (preferredAttributes.Bounds != originalAttributes.Bounds && heightRequiresLayoutInvalidate)
				{
					return true;
				}
			}
