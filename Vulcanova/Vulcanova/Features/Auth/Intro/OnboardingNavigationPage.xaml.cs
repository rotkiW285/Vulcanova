...
{
    private readonly IDisposable _errorHandlerDisposable;

    public OnboardingNavigationPage(IPopupNavigation navigationService)
    {
        InitializeComponent();

        _errorHandlerDisposable = Interactions.Errors.RegisterHandler(async ctx =>
        {
    ...
    {
        _errorHandlerDisposable?.Dispose();
        base.OnDisappearing();
    }
}